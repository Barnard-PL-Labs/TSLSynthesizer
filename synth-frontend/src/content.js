...
function setEffects(mouseX, mouseY, effectType) {
  if (effectType == "delay") {
    ...
    filter3.frequency.value =
      FilterY / 100 * (22050 * 0.2) * generateRandomNumber(0.1, 1.7);
...
function createOscillatorInObject(type, frequency) {
  ...
  if (oscillators.hasOwnProperty(oscillator_id)) {
    i = i + 1;
    createSingleOscillator(
      i,
      type,
      frequency,
      i * generateRandomNumber(0.4, 0.9)
    );
  } else {
    createSingleOscillator(
      i,
...
function windowLoadHandler() {
  ...
  keyboard.keyUp = function(note, frequency) {
    ...
    var delayTime =
      delayEffect.delayTime.value *
      2 *
      60 *
      60 *
      (delayFeedback.gain.value * 5.2);
    setTimeout(function() {
